...
use spacex_runtime::{self, opaque::Block, RuntimeApi};
...
type FullClient = sc_service::TFullClient<Block, RuntimeApi, SpacexExecutor>;
...
pub fn new_partial(config: &Configuration) -> Result<
    sc_service::PartialComponents<
        ...
        (
            impl Fn(
                spacex_rpc::DenyUnsafe,
                spacex_rpc::SubscriptionTaskExecutor
            ) -> spacex_rpc::RpcExtension,
            (
    ...
    let (client, backend, keystore_container, task_manager) =
        sc_service::new_full_parts::<Block, RuntimeApi, SpacexExecutor>(&config)?;
    ...
    let rpc_extensions_builder = {
        ...
        move |deny_unsafe, subscription_executor| -> spacex_rpc::RpcExtension {
            let deps = spacex_rpc::FullDeps {
                client: client.clone(),
                pool: transaction_pool.clone(),
                select_chain: select_chain.clone(),
                deny_unsafe,
                babe: spacex_rpc::BabeDeps {
                    babe_config: babe_config.clone(),
                    shared_epoch_changes: shared_epoch_changes.clone(),
                    keystore: keystore.clone(),
                },
                grandpa: spacex_rpc::GrandpaDeps {
...
pub fn new_light(mut config: Configuration) -> Result<TaskManager, ServiceError> {
    let (client, backend, keystore, mut task_manager, on_demand) =
        sc_service::new_light_parts::<Block, RuntimeApi, SpacexExecutor>(&config)?;
    ...
    let light_deps = spacex_rpc::LightDeps {
    ...
    let rpc_extensions = spacex_rpc::create_light(light_deps);
